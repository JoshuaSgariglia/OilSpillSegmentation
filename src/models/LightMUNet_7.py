...
class SSMBlock(layers.Layer):
    """
    Mamba-style SSM block WITHOUT gating.
    The input projection generates the depthwise convolution kernel weights directly.
    """

    def __init__(self, channels, ssm_kernel_size=16):
        super().__init__()
        self.channels = channels
        self.ssm_kernel_size = ssm_kernel_size

        # Layer normalization before kernel generation
        self.norm = layers.LayerNormalization(epsilon=1e-5)

        # Input projection directly generates convolution kernel weights
        # Output shape: [B, H, W, channels * ssm_kernel_size]
        self.kernel_generator = layers.Dense(channels * ssm_kernel_size, use_bias=True)

        # Output projection to mix channels after convolution
    ...
    def call(self, inputs):
        ...
        # 2. Generate kernels for each channel and spatial position
        kernels = self.kernel_generator(x)  # [B, H, W, C * k]
        kernels = tf.reshape(kernels, [B, seq_len, C, self.ssm_kernel_size])
        kernels = tf.transpose(kernels, [0, 2, 3, 1])  # [B, C, k, L]

        # 3. Flatten spatial dims for convolution input
        input_seq = tf.reshape(inputs, [B, seq_len, C])  # [B, L, C]
        input_seq = tf.transpose(input_seq, [0, 2, 1])   # [B, C, L]

        # 4. Pad input sequence for causal convolution
        padding = tf.zeros([B, C, self.ssm_kernel_size - 1], dtype=input_seq.dtype)
        padded_seq = tf.concat([padding, input_seq], axis=-1)  # [B, C, L + k -1]

        # 5. Apply depthwise convolution per batch/channel using kernels
        # Unfortunately, TensorFlow does not support per-sample kernels natively,
        # so we'll do a batched multiply + sum trick (slow for large batches/sequences).
        # Here is a simple but not optimized implementation:

        # Extract sliding windows of size k along the sequence dimension
        windows = tf.signal.frame(padded_seq, frame_length=self.ssm_kernel_size, frame_step=1, axis=-1)  
        # [B, C, L, k]

        # Multiply windows by kernels and sum over kernel dim
        # kernels shape: [B, C, k, L], transpose last dims to match windows
        kernels_transposed = tf.transpose(kernels, [0, 1, 3, 2])  # [B, C, L, k]

        conv_out = tf.reduce_sum(windows * kernels_transposed, axis=-1)  # [B, C, L]

        # 6. Transpose back to [B, L, C]
        conv_out = tf.transpose(conv_out, [0, 2, 1])  # [B, L, C]

        # 7. Output projection
        out_seq = self.output_proj(conv_out)  # [B, L, C]

        # 8. Reshape back to [B, H, W, C]
