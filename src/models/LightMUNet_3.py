...
class SSMBlock(layers.Layer):
    ...
    def call(self, inputs):
        ...
        """
        # LayerNorm first (as in Mamba)
        x = self.norm(inputs)

        # Project to value and gate
        proj = self.input_proj(x)  # [B, H, W, 2C]
        value, gate = tf.split(proj, 2, axis=-1)  # Each [B, H, W, C]
        gate = tf.sigmoid(gate)

        # Reshape for Conv1D: merge batch and height, move width to axis 1
        batch_size = tf.shape(inputs)[0]
        height = tf.shape(inputs)[1]
        width = tf.shape(inputs)[2]
        channels = tf.shape(inputs)[3]

        value_reshaped = tf.reshape(value, [batch_size * height, width, channels])  # [B*H, W, C]

        # SSM kernel: depthwise Conv1D (causal)
        ssm_out = self.ssm_kernel_conv(value_reshaped)  # [B*H, W, C]

        # Restore original shape
        ssm_out = tf.reshape(ssm_out, [batch_size, height, width, channels])
