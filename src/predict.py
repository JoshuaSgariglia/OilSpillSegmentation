...
from config import DatasetRegistry, SaveFilename
import os 
...
class EvaluationSession:
    ...
    # Evaluate model
    @classmethod
    def evaluate_model(cls, image_paths: str, mask_paths: str, model: Model, model_dir_path: str, model_dir_name: str, logger: Logger):
        ...
        with open(os.path.join(model_dir_path, SaveFilename.EVALUATION), "w") as outfile:
    ...
    def __init__(self, image_paths: list[str], mask_paths: list[str], model_names: list[str], logger: Logger):
        ...
        for model_name in model_names:
                ...
                for model_dir_name in model_dir_names:
                    ...
                    if not os.path.isfile(os.path.join(model_dir_path, SaveFilename.EVALUATION.value)):
                    
                        # Load the model
                        model = load_model(os.path.join(model_dir_path, SaveFilename.MODEL.value))
                        ...
                        # Save metrics dictionary in JSOn file
                        SavesManager.save_json(os.path.join(model_dir_path, SaveFilename.EVALUATION.value), evaluation)
